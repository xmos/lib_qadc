...
/** 
 * @brief   Channel communication commands for adc_rheo_task(). Please OR this with
...
/**
 * Initialise a QADC rheostate reader instance and initialise the adc_pot_state structure. 
 ...
 *                          and conversion rate / mode. This must be initialised before passing to adc_rheo_init().
 * \param adc_rheo_state    Reference to the adc_rheo_state_t struct which contains internal state for the QADC. This
 ...
 * adc_rheo_init() must be called before this function.
 *
 * \param p_adc             An array of 1 bit ports used for conversion.
 ...
 *                      and conversion rate / mode. This must be initialised before by adc_rheo_init() before-calling this task.
 */
